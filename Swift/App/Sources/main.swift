...
public struct PGM60061: Solvable {
    ...
    func solution(_ n: Int, _ build_frame: [[Int]]) -> [[Int]] {
        ...
        func isColumnAvailable(_ y: Int, _ x: Int, _ status: [[Build]]) -> Bool {
            if y == 0 { return true }
            if (y > 0 && !hasColumn(y-1, x, status)) && (x > 0 && !hasBeam(y, x-1, status)) && !hasBeam(y, x, status) { return false }
            return true
        }
        ...
        for build in build_frame {
            ...
            if b == 1 {
                if a == 0 && isColumnAvailable(y, x,status) {
                    buildSomething(y, x, .column, &status)
                } else if a == 1 && isBeamAvailable(y, x, status) {
                ...
                if status[y][x] == .joint {
                    if a == 0 {
                        ...
                        if x > 0 && newStatus[y+1][x-1] == .beam && !isBeamAvailable(y+1, x-1, newStatus) { isPossible = false }
                        if newStatus[y+1][x] == .beam && !isBeamAvailable(y+1, x, newStatus) { isPossible = false }
                        if newStatus[y+1][x] == .column && !isColumnAvailable(y+1, x, newStatus) { isPossible = false }
                        if newStatus[y+1][x] == .joint && (!isBeamAvailable(y+1, x, newStatus) || !isColumnAvailable(y+1, x, newStatus)) { isPossible = false }
                        if isPossible { status = newStatus }
                    } else {
                        newStatus[y][x] = .column
                        var isPossible = true

                        if x > 0 && newStatus[y][x-1] == .beam && !isBeamAvailable(y, x-1, newStatus) { isPossible = false }
                        if newStatus[y][x+1] == .beam && !isBeamAvailable(y, x+1, newStatus) { isPossible = false }
                        if newStatus[y][x] == .column && !isColumnAvailable(y, x, newStatus) { isPossible = false }
                        if newStatus[y][x+1] == .column && !isColumnAvailable(y, x+1, newStatus) { isPossible = false }
                        if isPossible { status = newStatus }
                    }
                } else if status[y][x] == .column && a == 0 {
                    newStatus[y][x] = .blank
                    var isPossible = true
